...
/* window.calculateShippingPrice = function (productId, span, value) {
...
window.calculateShippingPrice = function (productId, span, value) {
    ...
    if (selectedItem) {
        ...
        const productShippingPrice = (shippingPrice).toFixed(2);
        span.innerHTML = `$${productShippingPrice}`;
    }

    // Recalculate and update the total shipping cost for all products
...
function calculateTotalShipping() {
    ...
    shippingPrice.innerHTML = `$${totalShippingPrice}`;
}
...
window.handleRadioClicked = function (productId, radio) {
    ...
    if (selectedItem) {
        ...
        radios.forEach(r => {
            if (r == radio) {
                r.checked = true;
                calculateShippingPrice(productId, shippingPrice, r.value);
...
window.load = function () {
    cart.forEach((product) => {
        ...
        if (savedChoise) {
            ...
            if (radio) {
                radio.checked = true;
                calculateShippingPrice(product.id, shippingPrice, savedChoise);
